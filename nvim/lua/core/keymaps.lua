...
return now(function()
  ...
  map('n', '<Leader>rr', ':source $MYVIMRC<CR>', { desc = 'Reload config' })
  
  -- Quick substitution mappings
  map('n', '<Leader>s', ':%s/<C-r><C-w>//g<Left><Left>', { desc = 'Search and replace word' })
  map('v', '<Leader>s', ':s///g<Left><Left><Left>', { desc = 'Search and replace selection' })
